        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Editar Livro: {{ $book->title }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('publisher_id') is-invalid @enderror" id="publisher_id" name="publisher_id" required>
                                ...
                                @foreach($publishers as $publisher)
                                    <option value="{{ $publisher->id }}" @if(old('publisher_id', $book->publisher_id) == $publisher->id) selected @endif>
                                        {{ $publisher->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if ($book->cover_image)
                            <div class="mb-3">
                                <p>Capa Atual:</p>
                                <img src="{{ asset('storage/' . $book->cover_image) }}" alt="Capa de {{ $book->title }}" height="150">
                            </div>
                        @endif
